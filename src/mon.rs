...
unsafe extern "system" fn get_proc(hmonitor: HMONITOR, _hdc: HDC, _rect: *mut RECT, _lparam: LPARAM) -> BOOL {
    ...
    if GetNumberOfPhysicalMonitorsFromHMONITOR(hmonitor, &mut mon_count as *mut u32).as_bool() {
        if mon_count > 0 {
            let mut mons = Vec::<PHYSICAL_MONITOR>::with_capacity(mon_count as usize);
            mons.set_len(mon_count as usize);
            if GetPhysicalMonitorsFromHMONITOR(hmonitor, &mut mons).as_bool() {
                ...
                for mon in mons {
                    ...
                    if DestroyPhysicalMonitor(mon.hPhysicalMonitor).as_bool() {
